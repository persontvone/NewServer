...
PPER_SOCKET_CONTEXT g_pContextList = NULL;		// linked list of context info structures
...
void __cdecl main(int argc, char* argv[])
{
	...
	while (g_bRestart)
	{
		...
		__finally {
			...
			if (g_pListenSocketContext) {
				while (!HasOverlappedIoCompleted((LPOVERLAPPED)&g_pListenSocketContext->pIOContext->Overlapped))
					Sleep(0);

				if (g_pListenSocketContext->pIOContext->SocketAccept != INVALID_SOCKET)
					closesocket(g_pListenSocketContext->pIOContext->SocketAccept);
				g_pListenSocketContext->pIOContext->SocketAccept = INVALID_SOCKET;

				//
				// We know there is only one overlapped I/O on the listening socket
				//
				if (g_pListenSocketContext->pIOContext)
					xfree(g_pListenSocketContext->pIOContext);

				if (g_pListenSocketContext)
					xfree(g_pListenSocketContext);
				g_pListenSocketContext = NULL;
			}
...
BOOL CreateAcceptSocket(BOOL fUpdateIOCP) {
	...
	if (fUpdateIOCP) {
		...
		nRet = WSAIoctl(
			...
			&g_pListenSocketContext->fnAcceptEx,
			sizeof(g_pListenSocketContext->fnAcceptEx),
			&bytes,
			NULL,
			NULL
		);
...
VOID CtxtListAddTo(PPER_SOCKET_CONTEXT lpPerSocketContext) {
	...
	if (g_pContextList == NULL) {
		...
		lpPerSocketContext->pCtxtBack = NULL;
		lpPerSocketContext->pCtxtForward = NULL;
		g_pContextList = lpPerSocketContext;
	}
	else {
